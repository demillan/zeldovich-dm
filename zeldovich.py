...
def make_gauss_init(pkinit, boxsize=512.0,ngrid=512,seed=314159,exactpk=True, smw=2.0):
    ...
    if (exactpk):
        dk = N.exp(2j*N.pi*rs.rand(ngrid*ngrid*(thirdim))).reshape((ngrid,ngrid,thirdim)).astype(N.complex)
    else:
        dk = N.empty((ngrid,ngrid,thirdim),dtype=N.complex)
        dk.real = rs.normal(size=ngrid*ngrid*(thirdim)).reshape((ngrid,ngrid,thirdim)).astype(N.float)
        dk.imag = rs.normal(size=ngrid*ngrid*(thirdim)).reshape((ngrid,ngrid,thirdim)).astype(N.float)
        dk /= N.sqrt(2.0)
...
def get_pos(fx, fy, fz, redshift, boxsize=128.0, ngrid=128):
    cell_len=N.float(boxsize)/N.float(ngrid)
