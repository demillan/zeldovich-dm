...
for i in tqdm(range(0, reps)):
    res = Parallel(n_jobs=num_cores)(delayed(init_field)(j, kmin, lmn_grid, pkinit) for j in range(0, len(lmn_grid)))
    tetha_klmn = [item[0] for item in res]
    delta = [item[1] for item in res]
