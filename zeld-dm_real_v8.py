...
mn_grid = np.array(np.meshgrid(m, n)).reshape(2, (2*M + 1)**2).T
...
reps = 100
deltax = np.zeros(shape=(N3, 3), dtype = np.float64)

for i in tqdm(range(0, N3)):
    for li in range(0, len(l)):
        for mni in range(0, len(mn_grid)):
            if ((l[li] == 0) & (mn_grid[mni][0] == 0) & (mn_grid[mni][1] == 0)):
                deltax[i] += 0
            else:
                ...
                deltax[i] += 2*np.real(delta_lmn * np.exp(1j * np.dot([l[li], mn_grid[mni][0], mn_grid[mni][1]], [x1[i], x2[i], x3[i]])))
